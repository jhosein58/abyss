...
use crate::{
    ast::{FunctionDef, Program, Span as AstSpan, StmtKind},
...
impl<'a> Parser<'a> {
    ...
    pub fn pack_filename(name: &str) -> u64 {
        let mut bytes = [0u8; 8];
    ...
    pub fn unpack_filename(packed: u64) -> String {
        let bytes = packed.to_le_bytes();
        let end = bytes.iter().position(|&x| x == 0).unwrap_or(8);
        String::from_utf8_lossy(&bytes[..end]).to_string()
    }

    pub fn get_ast_span(&self) -> AstSpan {
        let s = self.stream.current_span();
        let pos = self.source_map.position_from_span(&s, self.source).unwrap();
        AstSpan {
            line: pos.line as u32,
            col: pos.column as u32,
            file_id: Self::pack_filename(self.file_path),
        }
    }
    ...
    fn parse_definitions(&mut self, end_token: Option<TokenKind>) -> Program {
        ...
        loop {
            ...
            match self.stream.current().kind {
                ...
                TokenKind::Type => {
                    if let Some(ty_alias) = self.parse_type_alias(is_pub) {
                        type_aliases.push(ty_alias);
                    }
                }
