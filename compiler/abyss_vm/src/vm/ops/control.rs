...
use crate::vm::{
    core::AbyssVm,
    opcode::Instruction,
    types::{CallFrame, EXTERN_FUNC_TAG, REG_PTR_TAG},
};
...
pub fn call_dynamic(
    ...
    let target_val = unsafe { *registers_ptr.add(*bp + inst.b as usize) };

    if (target_val & EXTERN_FUNC_TAG) != 0 {
        let func_idx = (target_val & !EXTERN_FUNC_TAG) as usize;
        let arg_start_reg = inst.c;
        ...
        unsafe {
            for i in 0..arity {
                let raw_val = *registers_ptr.add(args_start_abs + i);
                if extern_func.is_pointer_args[i] {
                    if (raw_val & crate::vm::types::REG_PTR_TAG) != 0 {
                        arg_values[i] = raw_val;
                    } else if (raw_val as usize) < vm.heap.len() {
                        let ptr = vm.heap.as_mut_ptr().add(raw_val as usize);
                        arg_values[i] = ptr as u64;
        ...
        if let Some(ref host_fn) = extern_func.host_fn {
            let heap_slice =
                unsafe { std::slice::from_raw_parts_mut(vm.heap.as_mut_ptr(), vm.heap.len()) };
            result = host_fn(&arg_values[..arity], heap_slice);
        } else {
            #[cfg(feature = "ffi")]
            {
            ...
            #[cfg(not(feature = "ffi"))]
            {
                panic!("FFI disabled for function {}", extern_func.name);
            }
        }
        ...
        let target_ip = target_val as usize;
