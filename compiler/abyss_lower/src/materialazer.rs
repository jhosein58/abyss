...
pub fn lower_function<M: ModuleBuilder>(
    db: &mut Nexus,
    module: &mut M,
    symbol: SymbolId,
) -> M::FuncId {
    ...
    // TODO: insert function params to ctx
...
fn lower_expr<B: FunctionBuilder>(
    ...
    match kind {
        ...
        Hir::Var => {
            ...
            let symbol_type = get_type(db, symbol_hir_id);
        ...
        Hir::Block => {
            let nodes = db.get_list_flat(db.hir.lhs(id).0).to_owned(); // FIXME: remove allocation
...
fn lower_type<TB: TypeBuilder>(db: &Nexus, ty_id: TypeId, builder: &mut TB) -> TB::Type {
    match db.types.kind(ty_id) {
        ...
        TyKind::Unknown => builder.type_never(), // FIXME
